...
class ProfController extends Controller
{
    ...
    public function save(Request $request)
    {
        ...
        return redirect()->route('admin.show.all.prof')->with(['ajoute' => ' Enseignant est Bien Ajoute ']);
    ...
    public function editProf($id)
    {
        ...
        if(!$prof)
           redirect() -> route('admin.show.all.prof') -> with(['Erreur' => "Enseignant n'existe pas !!!"]);
    ...
    try {
        Enseignant::where('id',$request ->id) -> update(
            ...
            return redirect()->route('admin.show.all.prof')->with(['success' => ' Enseignant est Bein modifié ']);

        } catch (\Exception $ex) {
            //  // un message d'erreur  s'il y a pas de modification
            return redirect()->route('admin.add.prof')->with(['error' => 'There is somthing went wrong ']);
    }
...
public function deleteprof($id)
{
    $prof = Enseignant::find($id);
    if(!$prof)
       redirect() -> route('admin.show.all.prof') -> with(['error' => 'Enseignant no existe']);

       Enseignant::where('id',$id) -> delete();
       return redirect()->route('admin.show.all.prof')->with(['delete' => 'enseignant supprime avec succes']);
}
