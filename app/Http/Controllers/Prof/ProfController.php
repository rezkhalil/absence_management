...
class ProfController extends Controller
{
  ...
  try {
        ...
        return redirect()->route('prof.list.seance')->with(['success' => 'seance est Bien Ajoute']);

    } catch (\Exception $ex) {

        return redirect()->route('prof.create.seance')->with(['error' => 'Erreur!!! ']);
    }
...
public function saveAbsence(Request $request)
{
  ...
  try {
    ...
    Seance::where('id',$request->id_sea)->update(['active' => 1]);
       ...
       return redirect()->route('prof.list.seance')->with(['success' => 'absence est bien ajouter ']);
  } catch (\Exception $ex) {
      return $ex;
    return redirect()->route('prof.list.seance')->with(['error' => 'Erreur!!! ']);
  }
...
public function historiqueAbsence()
{
   ...
   return view('Enseignant.historiqueAbsence',compact('seances','matieres','absence','filiere'));
}
